...
import {changeLoadingStatusAC} from './state/loading-reducer';
...
export function TodolistWithRedux({todolist}: PropsType) {
    ...
    let isLoading = useSelector<AppRootStateType, boolean>(state => state.loading[todolist.id])

    const dispatch = useDispatch()


    const dispatchWithLoading = (action: TasksActionsType) => {
        dispatch(changeLoadingStatusAC(todolist.id, true))
        dispatch(action)
        setTimeout(() => dispatch(changeLoadingStatusAC(todolist.id, false)), 20000)
    }
