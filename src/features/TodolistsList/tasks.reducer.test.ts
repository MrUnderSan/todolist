...
test("correct task should be added to correct array", () => {
  ...
  const action = tasksThunks.addTask.fulfilled({ task }, "", { title: task.title, todolistId: task.todoListId });
...
test("status of specified task should be changed", () => {
  ...
  const action = tasksThunks.updateTask.fulfilled(taskPayload, "requestId", taskPayload);
...
test("title of specified task should be changed", () => {
  let taskPayload = { taskId: "2", domainModel: { title: "yogurt" }, todolistId: "todolistId2" };
  const action = tasksThunks.updateTask.fulfilled(taskPayload, "requestId", taskPayload);
