...
const slice = createSlice({
  ...
  reducers: {
    removeTask: (state, action: PayloadAction<{ taskId: string; todolistId: string }>) => {
      const todoTasks = state[action.payload.todolistId]
      const index = todoTasks.findIndex((t) => t.id === action.payload.taskId)
      if (index !== -1) todoTasks.splice(index, 1)
    },
    addTask: (state, action: PayloadAction<{ task: TaskType }>) => {
      const todoTasks = state[action.payload.task.todoListId]
      todoTasks.unshift(action.payload.task)
    },
    updateTask: (
      state,
      ...
      const todoTasks = state[action.payload.todolistId]
      const index = todoTasks.findIndex((t) => t.id === action.payload.taskId)
      if (index !== -1) todoTasks[index] = { ...todoTasks[index], ...action.payload.model }
    },
